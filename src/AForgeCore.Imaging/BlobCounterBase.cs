...
namespace AForgeCore.Imaging
{
    ...
    public abstract class BlobCounterBase
    {
        ...
        /// useful in conjunction with different routines from <see cref="AForgeCore.Math.Geometry"/>,
        ...
        private unsafe void CollectObjectsInfo( UnmanagedImage image )
        {
            ...
            for ( int j = 1; j <= objectsCount; j++ )
            {
                ...
                blob.CenterOfGravity = new AForgeCore.Point( (float) xc[j] / blobArea, (float) yc[j] / blobArea );
